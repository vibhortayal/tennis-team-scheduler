-- Migration 007: knockout stage support + standings exclusion fix
--
-- Adds to public.dashboard_matches:
--   - stage: 'group' (default) | 'quarterfinal' | 'semifinal' | 'final'
--   - knockout_slot: 'QF1'..'QF4', 'SF1', 'SF2', 'F' (null for group matches)
--
-- Also fixes public.recompute_team_standings() (first defined in 002) so it
-- skips matches flagged excluded_from_standings and any non-group-stage
-- match. The client-side scorer already honored excluded_from_standings;
-- the database trigger did not, so the team_standings table could drift.
--
-- Security / compatibility / rollback:
--  - Additive only: two new nullable-or-defaulted columns, no column drops.
--  - Function stays SECURITY DEFINER with search_path locked to public;
--    no RLS or grant changes.
--  - Existing rows get stage='group', knockout_slot=null (unchanged behavior).
--  - Rollback: re-apply 002_standings_trigger.sql, then
--    alter table public.dashboard_matches drop column stage,
--    drop column knockout_slot;
-- ---------------------------------------------------------------------------

-- New columns for knockout support (additive; existing rows unaffected)
alter table public.dashboard_matches
  ...
  add column if not exists knockout_slot text
    check (knockout_slot in ('QF1', 'QF2', 'QF3', 'QF4', 'SF1', 'SF2', 'F'));
    ...
    BEGIN
      FOR m IN
        ...
        WHERE  dm.status = 'Completed'
          ...
          AND  trim(dm.result) <> ''
          AND  dm.excluded_from_standings IS NOT TRUE
          AND  coalesce(dm.stage, 'group') = 'group'
