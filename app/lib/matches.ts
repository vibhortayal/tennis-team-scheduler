...
new Intl.DateTimeFormat('en-US', { weekday: 'short', month: 'short', day: 'numeric' }).format(new Date(`${d}T12:00:00`));
...
export const daysBetween = (from: string, to: string) => Math.floor((dayValue(to) - dayValue(from)) / 86400000);
...
export const teamIds = (m: Match, g: Group) => groups[g].filter(([id]) => matchIncludesTeam(m.matchup, id)).map(([id]) => id);
...
return (ids.includes(firstId) && ids.includes(secondId)) || (matchIncludesTeam(match.matchup, firstId) && matchIncludesTeam(match.matchup, secondId));
};

export const existingFixture = (allMatches: Match[], group: Group, firstId: string, secondId: string) =>
allMatches.find(match => isBlockingStatus(match.status) && isSameFixture(match, group, firstId, secondId));

export const matchesForTeam = (allMatches: Match[], group: Group, teamId: string) =>
allMatches.filter(match => (match.league_group || 'Group B') === group && isBlockingStatus(match.status) && matchIncludesTeam(match.matchup, teamId));

export const restGapAroundDate = (teamMatches: Match[], date: string) => {
const previous = teamMatches.filter(match => match.match_date < date).sort((a, b) => b.match_date.localeCompare(a.match_date))[0];
const nextMatch = teamMatches.filter(match => match.match_date > date).sort((a, b) => a.match_date.localeCompare(b.match_date))[0];
